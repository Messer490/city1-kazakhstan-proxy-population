...
\input{tables_tex/table2_core_validation.tex}
...
These weaker district rows should not be read as a collapse of the baseline claim. Rather, they show why a single internal aggregate benchmark is insufficient in this setting. The district reference package mixes source dates and precision levels, excludes some lower-bound rows from strict metrics, and still carries boundary warnings in Astana and Shymkent; it is therefore an independent but coarse administrative check rather than a direct counterpart to the city-total calibration table. Administrative mismatch, temporal mismatch, heterogeneous urban form, and differences in open-data support can all weaken agreement once cell-level predictions are aggregated to imperfect district references. The district layer remains valuable precisely because it is partial: it contributes one internal perspective within a broader validation design instead of functioning as the sole arbiter of correctness.
...
The full frozen feature configuration remained the strongest setting. It achieved a calibrated RMSE of 115.934 and a calibrated $R^2$ of 0.934, while the built-form-only variant was the strongest reduced ablation with a calibrated RMSE of 120.656 and a calibrated $R^2$ of 0.929. As in the core validation rows above, these metrics are computed against held-out weak targets after calibration rather than against observed cell-level census labels. The full feature configuration also showed a large calibration RMSE gain of 135.821.
...
\input{tables_tex/table5_ablation.tex}
