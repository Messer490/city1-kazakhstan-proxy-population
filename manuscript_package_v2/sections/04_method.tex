...
In the frozen v2 baseline, the selected production model is \emph{random forest}. This choice reflects the fixed evidence stack reported in the manuscript and is retained for the final production identity of the system. A linear \emph{ridge} baseline is evaluated as a comparator in the validation stage, but it is not used for the frozen production surface. The model is trained and evaluated strictly within the frozen v2 workflow so that all reported figures and tables remain traceable to the same reproducible package.
...
This frozen framing is central to the paper's contribution. City1 v2 is not introduced as a best-possible truth model, nor as a software demo centered on an interface. It is presented as a reproducible open-data baseline that combines weak supervision, official-total calibration, and multi-level validation for intra-urban analysis in data-scarce urban settings. In the manuscript package, the same frozen workflow is also responsible for the reported tables and figures, so that the narrative, quantitative results, and paper assets remain traceable to a single reproducible v2 evidence stack.

\begin{figure}[H]
\centering
\input{figures/figure_pipeline_schematic.tex}
